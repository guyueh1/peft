    ...
    **Attributes**:
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = MyLinear(adapter_name, in_features, out_features, bias=bias, **kwargs)
...
class MyLoraLayer:
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, dtype=None):
        ...
        if r > 0:
            self.lora_A = nn.Linear(self.in_features, r, bias=False, dtype=dtype)
            self.lora_B = nn.Linear(r, self.out_features, bias=False, dtype=dtype)
...
class MyLinear(nn.Linear, MyLoraLayer):
    # Lora implemented in a dense layer
    def __init__(
        self,
        adapter_name: str,
        ...
        **kwargs,
        ...
        nn.Linear.__init__(self, in_features, out_features, **kwargs)
        MyLoraLayer.__init__(self, in_features=in_features, out_features=out_features)
        # Freezing the pre-trained weight matrix
        self.weight.requires_grad = False
    ...
    def forward(self, x: torch.Tensor):
        previous_dtype = x.dtype
        if (not self.disable_adapters) and self.r > 0 and not self.merged:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
